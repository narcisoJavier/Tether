...
- **Immersive Terminal Shell Invariant** — When on the `/terminal` route, hide the persistent 5-branch `GlassBottomNavBar` (`bottomNavigationBar: isTerminal ? null : ...`) and eliminate inner `Scaffold.body` bottom padding so the VT100 shell occupies full screen height.
- **Virtual Keyboard Accessory Bar Invariant** — The mobile key ribbon must dynamically adapt to keyboard insets (`SafeArea(bottom: !isKeyboardOpen)`), attaching seamlessly to the software keyboard when typing and docking above the system gesture navigation pill when dismissed. Use `CTRL` (sending `0x03` SIGINT) and avoid duplicating launchers present in the top session header.
- **Modal Sheet Drag Handle Invariant** — Never render manual `Container` drag handles inside custom bottom sheet widgets. Material3 `BottomSheetThemeData(showDragHandle: true)` already injects the native top drag handle.
- **PTY Command Execution Invariant** — When writing direct commands to active SSH terminal stdin, always terminate with carriage return `\r` (ASCII 13) rather than `\n` to guarantee prompt line execution across Unix shells and PowerShell.
